...
struct pkt packet_queue_A[QUEUE_SIZE];
int window_base_A = 0;
int nextseqnum = 0;
int expect_seq = 0;
...
void A_output(struct msg message)
{
  ...
  if (nextseqnum < window_base_A + WINDOW_SIZE) {
    packet_queue_A[nextseqnum].seqnum = nextseqnum;
    packet_queue_A[nextseqnum].acknum = -1;
    memcpy(packet_queue_A[nextseqnum].payload, message.data, 20);
    packet_queue_A[nextseqnum].checksum = generate_checksum(packet_queue_A[nextseqnum]);
    number2= number2+1;

    tolayer3(ENTITY_A,packet_queue_A[nextseqnum]);
    printf("A Send Data %d\n", nextseqnum);
...
void A_input(struct pkt packet)
{
  if (check_checksum(packet)){
    ...
    if (packet.acknum == window_base_A){
        ...
        printf("A receive correct ack %d\n", packet.acknum);
        window_base_A = (window_base_A + 1) % QUEUE_SIZE;
        if (window_base_A == nextseqnum) {
          printf("A Time Stop\n");
          stoptimer(ENTITY_A);
        }
      ...
      printf("A receive incorrect ack %d, expect %d\n", packet.acknum, window_base_A);
    }
  }
  else{
...
void A_timerinterrupt()
{
  ...
  while(i!= nextseqnum){
    ...
    printf("A int Send Data %d\n", i);
    tolayer3(ENTITY_A,packet_queue_A[i]);
    i=(i+1) % QUEUE_SIZE; 
  }
}  
...
void A_init()
{
  memset(packet_queue_A, 0, sizeof(struct pkt)*QUEUE_SIZE);
  window_base_A = 0;
  nextseqnum = 0;
}
...
void B_input(struct pkt packet)
{
  ...
  if(check_checksum(packet)){
    ...
    if(packet.seqnum == expect_seq) {
      number4=number4+1;

      printf("B seq correct %d\n", packet.seqnum);
      tolayer5(ENTITY_B,packet.payload);
      expect_seq++;
    }
    else {
      printf("B seq %d error, expect %d\n", packet.seqnum, expect_seq);
    }
